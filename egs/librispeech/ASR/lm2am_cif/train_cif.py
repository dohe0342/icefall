...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        nnet_output, encoder_memory, memory_mask = model(
            feature, supervisions, warmup=warmup, texts=supervisions["text"], vis=vis, filenames=filenames,
        )
        ...
        if params.distill and not params.lm_tune:
            ...
            if params.cif:
                ...
                distill_loss = torch.mean((diagonal_elements - target) ** 2)
            else:
        ...
        if params.distill:
            ...
            if torch.any(~distill_loss_is_finite):
                ...
                # if either all ctc_loss or att_loss is inf or nan,
                # we stop the training process by raising an exception
                if torch.all(~distill_loss_is_finite):
                    raise ValueError(
                        "There are too many utterances in this batch "
                        "leading to inf or nan losses."
                    )
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        try:
            with torch.cuda.amp.autocast(enabled=params.use_fp16):
                loss, loss_info = compute_loss(
                    ...
                    vis=True if batch_idx % 500 == 0 else False
                )
            # summary stats
        ...
        if batch_idx % params.log_interval == 0:
            ...
            if wb_writer is not None:
                ...
                tot_loss.write_summary(wb_writer, "train/tot_", params.batch_idx_train)
        ...
        if batch_idx > 0 and batch_idx % params.valid_interval == 0:
            ...
            if wb_writer is not None:
                valid_info.write_summary(
                    wb_writer, "train/valid_", params.batch_idx_train
                )
...
def run(rank, world_size, args):
    ...
    if args.wandb and rank == 0:
        ...
        #wandb.tensorboard.patch(root_logdir=f"{params.exp_dir}/tensorboard", pytorch=True)
    ...
    if params.lm_tune:
        p_list = []
        for n, p in model.named_parameters():
            #if 'lm_tune' in n or 'lm_decoder' in n or 'quant' in n:
            if 'lm_tune' in n:
                p.requires_grad = True
                p_list.append(p)
            else:
                p.requires_grad = False
        optimizer = optim.Eve(p_list, lr=params.initial_lr)
    else:
    ...
    if args.wandb and rank == 0: wandb.finish()
