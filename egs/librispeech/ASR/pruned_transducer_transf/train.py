...
from decoder import Decoder, TransformerPrediction
...
def add_model_arguments(parser: argparse.ArgumentParser):
    parser.add_argument(
        ...
        type=str2bool,
        default=True,
        help="Using transformer decoder as prediction network"
    )
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        try:
            ...
            scaler.scale(loss).backward() ## equal as loss.backward()
            scheduler.step_batch(params.batch_idx_train)
            scaler.step(optimizer) ## equal as optimizer.step()
            scaler.update()
            optimizer.zero_grad()
        except:  # noqa
            display_and_save_batch(batch, params=params, sp=sp)
